...
public class AdapterProses extends RecyclerView.Adapter<AdapterProses.MyViewHolder>{
    List<PesananModel> mKategoriList;

    public AdapterProses(List <PesananModel> KategoriList) {
        this.mKategoriList = KategoriList;
    }

    @Override
    public AdapterProses.MyViewHolder onCreateViewHolder (ViewGroup parent,int viewType){
    ...
    @Override
    public void onBindViewHolder (AdapterProses.MyViewHolder holder, final int position){
//        Intent intent = (ProsesAct.ma.getIntent());
//        int value = intent.getIntExtra("idKategori",0);
//     //   int value = 8;
//        if (mKategoriList.get(position).getIdSiswa() == value){
//            if (mKategoriList.get(position).getStatus().equals("Proses")){
