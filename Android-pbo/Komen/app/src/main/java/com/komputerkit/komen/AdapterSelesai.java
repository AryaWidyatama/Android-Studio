...
public class AdapterSelesai extends RecyclerView.Adapter<AdapterSelesai.MyViewHolder>{
    List<PesananModel> mKategoriList;

    public AdapterSelesai(List <PesananModel> KategoriList) {
        this.mKategoriList = KategoriList;
    }

    @Override
    public AdapterSelesai.MyViewHolder onCreateViewHolder (ViewGroup parent, int viewType){
    ...
    @Override
    public void onBindViewHolder (AdapterSelesai.MyViewHolder holder, final int position){
//        Intent intent = (SelesaiNew.ma.getIntent());
//        int value = intent.getIntExtra("idKategori",0);
//        //   int value = 8;
//        if (mKategoriList.get(position).getIdSiswa() == value){
//            if (mKategoriList.get(position).getStatus().equals("Tolak")){
