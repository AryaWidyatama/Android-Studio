...
public class MakananAct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvJudul.setText("MAKANAN");
        headerjasa.setBackgroundTintList(ColorStateList.valueOf(getResources().getColor(R.color.makanan)));
        if (Build.VERSION.SDK_INT >= 21) {
            Window window = this.getWindow();
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.setStatusBarColor(this.getResources().getColor(R.color.makanan));
        }
...
//        navView.setSelectedItemId(R.id.ic_home);
        ...
        searchView.setOnQueryTextListener(new SearchView.OnQueryTextListener() {
            ...
            public boolean onQueryTextSubmit(String s) {
                if (s.length() > 0) {
                    ...
                    Call<GetBarang> kontakCall = mApiInterface.makanan(s);
                    kontakCall.enqueue(new Callback<GetBarang>() {
                        ...
                        public void onResponse(Call<GetBarang> call, Response<GetBarang>
                            ...
                            mAdapter = new AdapterMakanan(ListMenu);
                            mRecyclerView.setAdapter(mAdapter);
                        }

                        @Override
                        public void onFailure(Call<GetBarang> call, Throwable t) {
                            dialog=new Dialog(MakananAct.this);
            ...
            public boolean onQueryTextChange(String newText) {
                if (newText.length() > 0) {
                    ...
                    Call<GetBarang> kontakCall = mApiInterface.makanan(newText);
                    kontakCall.enqueue(new Callback<GetBarang>() {
                        @Override
                        public void onResponse(Call<GetBarang> call, Response<GetBarang>
                                response) {
    ...
    public void panggilRetrofit() {
        Call<GetBarang> kontakCall = mApiInterface.showmakanan();
        kontakCall.enqueue(new Callback<GetBarang>() {
            @Override
            public void onResponse(Call<GetBarang> call, Response<GetBarang>
                    response) {
