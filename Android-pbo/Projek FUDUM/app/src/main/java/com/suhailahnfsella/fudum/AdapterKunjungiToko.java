...
public class AdapterKunjungiToko extends RecyclerView.Adapter<AdapterKunjungiToko.MyViewHolder>{

    List<Menu> mKategoriList;

    public AdapterKunjungiToko(List <Menu> KategoriList) {
    ...
    @Override
    public void onBindViewHolder (AdapterKunjungiToko.MyViewHolder holder, final int position){
        Intent intent = (KunjungiToko.ma.getIntent());
        int value = intent.getIntExtra("IdToko", 0);

        if (mKategoriList.get(position).getIdtoko() == value){
